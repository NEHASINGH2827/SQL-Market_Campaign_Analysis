...
--1. DIFFERENT TYPES OF PAINTS PROVIDED BY THE COMPANY.
SELECT COUNT(DISTINCT(ITEM_CATEGORY)) CATEGORY_COUNT FROM ITEM;

--2. DIFFERENT COUPON TYPES THAT ARE OFFERED.
SELECT COUNT(DISTINCT(COUPON_TYPE)) COUPON_COUNT FROM COUPONMAPPING;

--3. STATES WHERE THE COMPANY IS CURRENTLY DELIVERING ITS PRODUCT AND SERVICES.
SELECT COUNT(DISTINCT(STATE)) STATE_COUNT FROM CITY;

--4. DIFFERNT ORDER TYPES.
SELECT DISTINCT(ORDER_TYPE) ORDERTYPE_COUNT FROM CUSTOMERTRANSACTIONDATA;

--5. IDENTIFY TOTAL NUMBER OF SALES HAPPENED BY:

--YEARLY BASIS
SELECT YEAR(PURCHASE_DATE) YEAR,COUNT(TRANS_ID) TRANS_COUNT FROM CUSTOMERTRANSACTIONDATA
GROUP BY YEAR(PURCHASE_DATE)

--QUARTERLY BASIS
SELECT DATEPART(QUARTER,PURCHASE_DATE) QUARTER,
COUNT(TRANS_ID) TRANS_COUNT
FROM CUSTOMERTRANSACTIONDATA
GROUP BY DATEPART(QUARTER,PURCHASE_DATE);

-- YEARLY AND MONTHLY BASIS
SELECT   DATEPART(YEAR, PURCHASE_DATE) YEAR,
DATEPART(MONTH, PURCHASE_DATE) MONTH,
COUNT(TRANS_ID) TRANS_COUNT
FROM CUSTOMERTRANSACTIONDATA
GROUP BY  DATEPART(YEAR, PURCHASE_DATE), DATEPART(MONTH, PURCHASE_DATE)

--6. IDENTIFY THE TOTAL PURCHASE ORDER BY:

-- PRODUCT CATEGORY
SELECT ITEM_CATEGORY, ROUND(SUM(C.PURCHASEAMOUNT),2)  TOTAL_SALES
FROM
CUSTOMERTRANSACTIONDATA C
INNER JOIN ITEM I
...
SELECT DATEPART(YEAR, PURCHASE_DATE) YEAR,
DATEPART(QUARTER, PURCHASE_DATE) QUARTER,
ROUND(SUM(PURCHASEAMOUNT),2) TOTAL_SALES
FROM CUSTOMERTRANSACTIONDATA
GROUP BY DATEPART(YEAR, PURCHASE_DATE),
DATEPART(QUARTER, PURCHASE_DATE)

--ORDER TYPE
SELECT ORDER_TYPE, ROUND(SUM(PURCHASEAMOUNT),2) TOTAL_SALES
FROM CUSTOMERTRANSACTIONDATA
GROUP BY ORDER_TYPE;

--CITY TIER

SELECT CITYTIER, SUM(PURCHASEAMOUNT) FROM CITY CT
INNER JOIN CUSTOMER CS
ON CT.CITY_ID = CS.CITY_ID
INNER JOIN CUSTOMERTRANSACTIONDATA CD
ON CD.CUST_ID = CS.CUSTOMER_ID
GROUP BY CITYTIER;

--7. IDENTIFY TOTAL NO. OF TRANSACTIONS WITH CAMPAIGN COUPON VS WITHOUT CAMPAIGN COUPON

SELECT 'PURCAHSE_WITHOUT_COUPON' AS 'COUPON_STATUS', COUNT(*) NUM_OF_TRANS
FROM CUSTOMERTRANSACTIONDATA 
WHERE COUPON_ID IS NULL
UNION ALL
SELECT 'PURCAHSE_WITH_COUPON' AS 'COUPON_STATUS',COUNT(*) NUM_OF_TRANS FROM CUSTOMERTRANSACTIONDATA 
WHERE COUPON_ID IS NOT NULL;

--8. IDENTIFY THE NO. OF POTENTIAL LEADS (CUSTOMERS) WHO HAVE MADE A PURCHASE BUT HAVE NOT USED THE CAMPAIGN COUPON

SELECT C.CUSTOMER_ID, C.NAME,
COUNT(TRANS_ID) 'NO_OF_PURCHASE'
FROM CUSTOMER C
...
--9. WHICH CUSTOMERS HAVE USED THE MOST COUPONS AND WHAT IS THE TOTAL PRICE

SELECT TOP(1)
C.CUSTOMER_ID, C.NAME,
COUNT(COUPON_ID) AS 'NUM_OF_COUPONS',
SUM(PURCHASEAMOUNT) AS 'TOTAL_PRICE'
FROM CUSTOMER C
INNER JOIN CUSTOMERTRANSACTIONDATA CT
ON C.CUSTOMER_ID = CT.CUST_ID
WHERE COUPON_ID IS NOT NULL
GROUP BY C.CUSTOMER_ID, C.NAME
ORDER BY 3 DESC, 4 DESC --3,4 IS THE COLUMN NUM.

--10. CUSTOMER IN WHICH CITY HAS THE HIGHEST NUMBER OF TRANSACTIONS AND WHAT IS THE TOTAL.

SELECT TOP(1) CITY_NAME,
COUNT(TRANS_ID) 'TOTAL_TRANS',
SUM(PURCHASEaMOUNT) 'TOTAL_SALES'
...
--11. CREATE A VIEW FOR TOP 10 CUSTOMERS BY PURCHASING AMOUNT
----------------------------------------------------------------------
GO
CREATE VIEW VW_TOP10CUSTOMERPURCHASE AS
 (SELECT TOP(10)  CT.CUST_ID, C.NAME, SUM(PURCHASEAMOUNT) AS 'TOTAL_SALES'
FROM CUSTOMERTRANSACTIONDATA CT
INNER JOIN CUSTOMER C
ON CT.CUST_ID = C.CUSTOMER_ID
...
--12. CREATE A VIEW FOR TOP 5 CUSTOMERS BY ORDER TYPE BASED ON TOTAL SALES

GO
CREATE VIEW VW_TOP5 AS
(SELECT TOP(5) C.NAME,CUST_ID, SUM(PURCHASEAMOUNT) AS 'TOTAL_SALES', ORDER_TYPE
FROM CUSTOMERTRANSACTIONDATA CT
INNER JOIN CUSTOMER C
ON C.CUSTOMER_ID = CT.CUST_ID
WHERE ORDER_TYPE IN ('HOUSEHOLD', 'INDUSTRIAL')
GROUP BY C.NAME, CUST_ID, ORDER_TYPE
ORDER BY 3 DESC)
GO
